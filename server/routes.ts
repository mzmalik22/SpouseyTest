...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/users/invite", async (req, res) => {
    ...
    if (currentUser.partnerId) {
      return res
        .status(400)
        .json({ message: "You already have a partner connected" });
    }
  ...
  app.post("/api/messages/refine", async (req, res) => {
    ...
    try {
      ...
      const refinedMessage = await refineMessage(
        message,
        vibe,
        userNickname,
        partnerNickname
      ...
      if (error.code === "insufficient_quota") {
        errorMessage =
          "OpenAI API quota exceeded. Using original message instead.";
      } else if (error.status === 429) {
        errorMessage =
          "OpenAI API rate limit reached. Using original message instead.";
      } else if (error.message && error.message.includes("API key")) {
        errorMessage =
          "OpenAI API key configuration issue. Using original message instead.";
      }
  ...
  app.post("/api/messages/refine-all-vibes", async (req, res) => {
    ...
    try {
      ...
      const result = await refineMessageAllVibes(
        message,
        userNickname,
        partnerNickname
      );
      ...
      // if (!process.env.OPENAI_API_KEY) {
      //   errorMessage =
      //     "OpenAI API key not configured. Using original messages instead.";
      // } else

      if (error.code === "insufficient_quota") {
        errorMessage =
          "OpenAI API quota exceeded. Using original messages instead.";
      } else if (error.status === 429) {
        errorMessage =
          "OpenAI API rate limit reached. Using original messages instead.";
      } else if (error.message && error.message.includes("API key")) {
        errorMessage =
          "OpenAI API key configuration issue. Using original messages instead.";
      }
  ...
  app.post("/api/onboarding", async (req, res) => {
    ...
    if (!maritalStatus || !relationshipCondition) {
      return res.status(400).json({
        message: "Marital status and relationship condition are required",
      });
    }
  ...
  app.post("/api/user/nickname", async (req, res) => {
    ...
    if (!nickname && !partnerNickname) {
      return res
        .status(400)
        .json({ message: "At least one nickname must be provided" });
    }
  ...
  app.post("/api/user/therapist-status", async (req, res) => {
    ...
    if (seesTherapist === undefined) {
      return res
        .status(400)
        .json({ message: "Therapist status must be provided" });
    }
    ...
    try {
      ...
      if (!updatedUser) {
        console.log(
          "User not found for therapist status update:",
          currentUser.id
        );
        return res.status(404).json({ message: "User not found" });
      }
      ...
      await storage.createActivity({
        userId: currentUser.id,
        type: "profile_update",
        description: `Updated therapist status: ${
          seesTherapist
      ...
      return res
        .status(500)
        .json({ message: "Failed to update therapist status" });
    }
  });
  ...
  app.get("/api/coaching/sessions", async (req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ message: "Failed to fetch coaching sessions" });
    }
  });
  ...
  app.get("/api/coaching/sessions/:id", async (req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ message: "Failed to fetch coaching session" });
    }
  });
  ...
  app.post("/api/coaching/sessions", async (req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ message: "Failed to create coaching session" });
    }
  });
  ...
  app.patch("/api/coaching/sessions/:id", async (req, res) => {
    try {
      ...
      if (Object.keys(updateData).length > 0) {
        const updatedSession = await storage.updateCoachingSession(
          sessionId,
          updateData
        );
      ...
      return res
        .status(500)
        .json({ message: "Failed to update coaching session" });
    }
  });
  ...
  app.delete("/api/coaching/sessions/:id", async (req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ message: "Failed to delete coaching session" });
    }
  });
  ...
  app.get("/api/coaching/sessions/:id/messages", async (req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ message: "Failed to fetch session messages" });
    }
  });
  ...
  app.post("/api/coaching/sessions/:id/messages", async (req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ message: "Failed to create session message" });
    }
  });
  ...
  app.post("/api/coaching/ai-response", async (req, res) => {
    try {
      ...
      if (!messageHistory || !Array.isArray(messageHistory)) {
        return res.status(400).json({
          message: "Message history is required and must be an array",
        });
      }
        ...
        const messages = [
          {
            ...
            empathetic advice for relationships. This conversation is about: "${
              session.title
            }" in the 
            category: "${session.category}".
            
            The user's name is ${
              currentUser.firstName || "the user"
            } and their relationship status is 
            ${currentUser.maritalStatus || "unspecified"}.
            
            If they mention their partner, their partner's name is ${
              currentUser.partnerNickname || "their partner"
            }.
  ...
  app.post("/api/calendar/integrations", async (req, res) => {
    ...
    try {
      ...
      if (!name || !type || !provider || !visibility || !color) {
        return res
          .status(400)
          .json({ message: "Missing required calendar fields" });
      }
  ...
  app.get("/api/calendar/integrations/:id", async (req, res) => {
    ...
    try {
      ...
      if (!integration) {
        return res
          .status(404)
          .json({ message: "Calendar integration not found" });
      }
      ...
      return res
        .status(500)
        .json({ message: "Failed to fetch calendar integration" });
    }
  });
  ...
  app.patch("/api/calendar/integrations/:id", async (req, res) => {
    ...
    try {
      ...
      const updatedIntegration = await storage.updateCalendarIntegration(
        integrationId,
        req.body
      );
      if (!updatedIntegration) {
        return res
          .status(404)
          .json({ message: "Failed to update calendar integration" });
      }
  ...
  app.delete("/api/calendar/integrations/:id", async (req, res) => {
    ...
    try {
      ...
      if (!success) {
        return res
          .status(500)
          .json({ message: "Failed to delete calendar integration" });
      }

      return res
        .status(200)
        .json({ message: "Calendar integration deleted successfully" });
    } catch (error) {
  ...
  app.get("/api/calendar/partner-events", async (req, res) => {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ message: "Failed to fetch partner calendar events" });
    }
  });
  ...
  app.get("/api/calendar/events/:id", async (req, res) => {
    ...
    try {
      ...
      if (event.creatorId !== currentUser.id) {
        ...
        if (
          !currentUser.partnerId ||
          event.creatorId !== currentUser.partnerId
        ) {
          return res.status(403).json({ message: "Access denied" });
        }
      ...
      return res
        .status(500)
        .json({ message: "Failed to fetch calendar event" });
    }
  });
  ...
  app.patch("/api/calendar/events/:id", async (req, res) => {
    ...
    try {
      ...
      if (!updatedEvent) {
        return res
          .status(404)
          .json({ message: "Failed to update calendar event" });
      }
  ...
  app.get("/api/calendar/tasks/event/:eventId", async (req, res) => {
    ...
    try {
      ...
      if (!task) {
        return res
          .status(404)
          .json({ message: "No task found for this event" });
      }
  ...
  app.post("/api/calendar/tasks", async (req, res) => {
    ...
    if (!eventId || !dueDate) {
      return res
        .status(400)
        .json({ message: "Event ID and due date are required" });
    }
      ...
      if (assigneeId && assigneeId !== currentUser.id) {
        await storage.createNotification({
          ...
          content: `${
            ...
            currentUser.firstName ||
            currentUser.username
          } assigned you a task: ${event.title}`,
        ...
        return res.json([
          {
            ...
            description:
              "Complete your profile to start receiving personalized relationship insights.",
            severity: "low",
            actionItem:
              "Finish the onboarding process to tell us about your relationship.",
            createdAt: new Date(),
          },
        ]);
      }
